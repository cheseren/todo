...
class _CategoryListState extends State<CategoryListPage> {
  ...
  Future<void> fetchCategories() async {
    return await controller.fetchCategories(_searchCtl.text);
  }
              ...
              child: Obx(() {
                if (controller.fetching.value == true)
                  ...
                  return LazyLoadScrollView(
                    onEndOfPage: () =>
                        controller.fetchMoreProducts(_searchCtl.text),
                    child: RefreshIndicator(
                        onRefresh: () async {
                          await controller.refreshCategories(_searchCtl.text);
                        },
                        child: itemList(controller.categories)),
                  );
              }),
            )
          ],
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return categoryWidget(
            controller.categories[index], widget.someString, context);
      },
    );
  }
        ...
        onChanged: (val) {
          if (val.length >= 1) {
            _debouncer.run(() async {
              await controller.fetchCategories(_searchCtl.text);
            });
          }
        },
